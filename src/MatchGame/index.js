...
class MatchGame extends Component {
  ...
  onClickImage = id => {
    const {randomTopImage, time} = this.state

    if (time > 0) {
      if (randomTopImage.id === id) {
        this.setState(prevState => ({
          score: prevState.score + 1,
          randomTopImage:
            imagesList[Math.ceil(imagesList.length * Math.random()) - 1],
        }))
      } else {
        this.finishGameAndRenderScoreCard()
      }
    } else {
      this.finishGameAndRenderScoreCard()
    }
  ...
  render() {
    const {activeTab, score, time, randomTopImage, isGameRunning} = this.state

    const filteredResults = imagesList.filter(
      eachFruit => eachFruit.category === activeTab,
    )
            ...
            <ul className="tabs-container">
              {tabsList.map(eachTab => (
                <TabItem
                  key={eachTab.tabId}
                  tabDetails={eachTab}
                  onChangingFruits={this.onChangingFruits}
